...
from rdkit import Chem  # Import RDKit's Chem module for handling chemical data
from rdkit.Chem import AllChem  # Correct import for embedding molecules
    ...
    **STATS:**
    ...
    def process(self) -> None:
        ...
        for line in dataset:
            ...
            if not mol:
                warnings.warn(f"Invalid SMILES '{smiles}' skipped.")
                continue
            # mol = Chem.AddHs(mol)
            if mol.GetNumConformers() == 0:
                AllChem.EmbedMolecule(mol)  # Generate 3D coords if none exist
            positions = mol.GetConformer().GetPositions()
            atomic_numbers = [atom.GetAtomicNum() for atom in mol.GetAtoms()]

            ys = [float(y) if y else float('NaN') for y in labels]
